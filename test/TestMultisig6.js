...
const MinePool2 = artifacts.require('PhxDoubleFarmVer2');
...
contract('MinePoolProxy', function (accounts){
  ...
  before("init", async()=>{
    ...
    phxfarmupproxyinst = phxfarmproxyinst;
...
/////////////////////////////////////////////////////////////////////////////////
  ...
  it("[0020] multisig for upgrade,should pass", async()=>{
    ...
    let implv2 = await MinePool2.new(mulSiginst.address);

    let msgData = phxfarmupproxyinst.contract.methods._upgradeTo(implv2.address).encodeABI();
    ...
    res = await testViolation("multiSig getBackPhx: This tx is not aprroved",async function(){
      await phxfarmupproxyinst._upgradeTo(implv2.address,{from:operator0});
    });
    ...
    let v2 = await phxfarmproxyinst.getVersion();
    assert.equal(v2,2);
  })
